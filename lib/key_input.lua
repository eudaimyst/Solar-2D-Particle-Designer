...
local function onKeyEvent(event)
	...
	if (activeInputField) then
		...
		for i = 1, #inputKeys do
			local keysToSend = inputKeys[i].keyName
			--set the default keys to keyname unless overriden
		...
		if (event.keyName == "enter" and event.phase == "down") then
			activeInputField:inputComplete()
		end
		if (event.keyName == "escape" and event.phase == "down") then
			activeInputField:inputCancel()
		end
		if (event.keyName == "deleteBack" and event.phase == "down") then
			activeInputField:inputDelete()
		end
	end

	if ((event.keyName == "w" or event.keyName == "up") and event.phase == "down") then
		upPressed = true
	end
	if ((event.keyName == "w" or event.keyName == "up") and event.phase == "up") then
		upPressed = false
	end

	if ((event.keyName == "s" or event.keyName == "down") and event.phase == "down") then
		downPressed = true
	end
	if ((event.keyName == "s" or event.keyName == "down") and event.phase == "up") then
		downPressed = false
	end
	...
	if (event.isCtrlDown) then
		...
		if ((event.keyName == "d" or event.keyName == "right") and event.phase == "up") then
			rightPressed = false
		end
	end
